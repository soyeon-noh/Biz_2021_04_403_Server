...
@WebServlet("/rent/*") // 'rent로 시작하는 모든 요청을 니가 처리해라' 라는 뜻
public class BookRentController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(subPath.equals("/seq")) {
			...
			if(strSeq == null || strSeq.equals("")) { //똑같지만 "" 만 하면 null 오류 날때가 있대
				...
				// Rendering을 하라
					...
					//	 톰켓이 하는 일이다.)
				// 	wepapp/WEB-INF/view/book.jsp 파일을 읽어서
				// 	Java 코드로 변환하고, 실행할 준비를 하라
				RequestDispatcher disp			// 사용자가 요청한 결과를 ()경로에 보내라
				= app.getRequestDispatcher("/WEB-INF/views/book.jsp");  // BOOK이랑 book.jsp 이랑 쪼물딱
			...
			if(bu_name == null || bu_name.equals("")) {
				...
				// Service 에서 전달된 데이터가 잘 왔나 test code
