...
public class FoodServiceImplV1 implements FoodService {
	// DB와 연동해야하기 때문에 가장먼저 설정해야할 것이 있다!
	protected Connection dbConn;

	public FoodServiceImplV1() {
		dbConn = DBContract.getDBConnection(); // 이것만설정해주면 DB사용할 준비 OK
	}
	...
	protected List<FoodDTO> select(PreparedStatement pStr) throws SQLException {
		...
		ResultSet rSet = pStr.executeQuery(); // sql실행 결과를담기
	...
	public List<FoodDTO> findByFName(String fd_name) {
		...
		try {
			...
			pStr = dbConn.prepareStatement(sql);
			pStr.setString(1, fd_name.trim()); //매개변수로 보내고
			List<FoodDTO> foodList = this.select(pStr); // select에게...뭐..?
