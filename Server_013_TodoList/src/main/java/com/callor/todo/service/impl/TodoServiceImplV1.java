...
public class TodoServiceImplV1 implements TodoService{
	...
	protected List<Map<String,Object>> select(ResultSet rSet) throws SQLException {
		...
		while(rSet.next()) {
			...
			Map<String,Object> tdVO
				= new HashMap<String,Object>(); // Map은 느리지만.. 그나마빠른게 HashMap이다
			
			// 칼럼 개수만큼 반복문 실행
			for(int index = 0; index < columns; index++) {
				
				// 메타데이터에서 
				//		index 위치의 칼럼명을 getter로 get!
				//	(참고 : index 값은 1부터 시작한다)
				String colName = md.getColumnName(index + 1);
				
				// ResultSet으로부터
				// index 위의 칼럼에 저장된 실제데이터 getter
	...
	public List<Map<String, Object>> selectAll() {
		...
		// 완료되지 않는(td_edate 값이 없는) 데이터를 우선 보이고
		// INSERT된 순서로 보여라.
		// 이렇게 정렬하면 미완료된 TODO가 먼저 출력되게 된다.
	...
	public Integer insert(Map<String, Object> vo) {
		...
		try {
			pStr = dbConn.prepareStatement(sql);
			// pStr.setObject(1, vo.get("td_sdate")); 와 같다.
	...
	// 사용하지 않을 코드지만 주석이 아까우니까!
	@SuppressWarnings("unused") // 이 method는 안쓸거야! 경고보이지마!
	private void viewSelect(ResultSet rSet) throws SQLException {
		/* ResultSet으로 DB로 부터 수신한 데이터에서
		...
		for(int i = 0; i < columns; i++) {
			System.out.print(md.getColumnName( i+1 ) + "\t"); //1번부터다
		}
		System.out.println("\n" + "-".repeat(50));
